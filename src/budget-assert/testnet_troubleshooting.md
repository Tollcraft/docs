...
- **`MetricsExtraction`** (`Warning: Failed to extract metrics for ...`) — the RPC responded successfully, but the expected `SorobanTransactionData` fields weren't where the tool expected them (a Protocol/SDK mismatch is the likely cause — see the [supported-versions table](reference.md#⚙️-supported-versions-compatibility)).
